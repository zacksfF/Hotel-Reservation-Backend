...
func AddBooking(store *storeer.Store, uid, rid primitive.ObjectID, from, till time.Time) *types.Booking {
...
func AddRoom(store *storeer.Store, size string, ss bool, price float64, hid primitive.ObjectID) *types.Room {
	room := &types.Room{
		Size:    size,
		Seaside: ss,
		Price:   price,
...
func AddHotel(store *storeer.Store, name string, loc string, rating int, rooms []primitive.ObjectID) *types.Hotel {
	var roomIDS = rooms
	if rooms == nil {
...
func AddUser(store *storeer.Store, fn, ln string, admin bool) *types.User {
